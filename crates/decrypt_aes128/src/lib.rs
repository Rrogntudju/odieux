...
pub fn decrypt_aes128(key: &[u8], iv: &[u8], data: &[u8]) -> Result<Vec<u8>> {
    ...
    let cipher = Aes128Cbc::new_from_slices(&key, &iv).context("Échec de la création du chiffre AES-128 CBC")?;
    Ok(cipher.decrypt(&mut encrypted_data).context("Échec de la décryption")?.to_vec())
}
...
fn test_demo_decrypt() {
    ...
    let data = [0xDA, 0x52, 0xF9, 0x7B, 0xAB, 0xAE, 0x0A, 0x79, 0x7F, 0x1C, 0x11, 0xEC, 0xB2, 0x09, 0x9F, 0xB0];
