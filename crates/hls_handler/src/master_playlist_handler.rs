...
impl MasterPlaylistHandler {
    pub fn new(url: Url, m3u8: &str) -> Result<Self> {
        ...
        let stream_inf_tag = track_assert_some!(master_playlist.stream_inf_tags().get(0), ErrorKind::InvalidInput);
        let media_playlist_url = track!(Url::options().base_url(Some(&url)).parse(stream_inf_tag.uri()).map_err(Error::from))?;
        ...
        Ok(MasterPlaylistHandler { media_playlist_handler })
    }
    ...
    pub fn handle_data(&mut self, action_id: ActionId, data: &[u8], fetch_duration_ms: u32) -> Result<()> {
        track!(self.media_playlist_handler.handle_data(action_id, data, fetch_duration_ms))
    }
